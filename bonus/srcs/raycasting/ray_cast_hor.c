...
/*   Updated: 2023/06/20 11:37:06 by woumecht         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    ray_cast_hor(t_cub *cub)
{
    ...
    x = 0;
    if (cub->ph_inter_x <= 0)
        (cub->ph_inter_x = CARRE, x = 1);
    else if (cub->ph_inter_x >= x_size(cub) * CARRE)
        (cub->ph_inter_x = x_size(cub) * CARRE - CARRE, x = 1);
    ...
    if (x == 1)
        return (1);
    if (cub->map[(int)((cub->ph_inter_y) / CARRE)][(int)((cub->ph_inter_x) / CARRE)] == '1' 
        || cub->map[(int)((cub->ph_inter_y) / CARRE)][(int)((cub->ph_inter_x) / CARRE)] == '2')
        return (1);
    else
    {
